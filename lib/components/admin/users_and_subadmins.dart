...
class UsersAndSubAdmins extends StatefulWidget {
  const UsersAndSubAdmins({super.key});

  @override
  State<UsersAndSubAdmins> createState() => _UsersAndSubAdminsState();
}

class _UsersAndSubAdminsState extends State<UsersAndSubAdmins> {
  int currentPage = 1;
  int totalPages = 1;
  int limit = 50;
  ...
  Widget buildBody() {
    ...
    return RefreshIndicator(
      onRefresh: fetchUsersAndSubAdmins,
      child: Padding(
                ...
                child: DataTable(
                  ...
                  rows: subAdmins.map((subAdmin) {
                    ...
                    return DataRow(
                      ...
                      selected: selectedUserSubAdmins.contains(subAdmin.id),
                      onSelectChanged: (isItemSelected) {
                        setState(() {
                          if (isItemSelected == true) {
                            selectedUserSubAdmins.add(subAdmin.id);
                          } else {
                            selectedUserSubAdmins.remove(subAdmin.id);
                          }
                        });
                      },
                    );
              ...
              itemBuilder: (BuildContext context) {
                return [
                  const PopupMenuItem(
                    ...
                    child: Text('Create User & SubAdmin'),
                  ),
                ];
              },
            )
